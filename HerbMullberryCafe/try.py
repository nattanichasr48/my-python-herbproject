...
# ... (ฟังก์ชัน Admin Edit Menu, Add Menu, Display Admin Menu Items ยังคงเดิม) ...
...
def display_admin_menu_items(root, category, bg_images, item_container):
    ...
    col_width = 200; start_x = 15; start_y = 10 
    for i, item in enumerate(menu_list):                         
        col = i % 4; x = start_x + col * (col_width + 20)
        item_frame = tk.Frame(item_container, bg="#ffffff", width=col_width, height=300, bd=0, highlightthickness=0); item_frame.place(x=x, y=start_y)
        ...
        tk.Label(item_frame, text=f"{item[MENU_PRICE_IDX]} ฿", font=("Arial", 16, "bold"), fg="#699039", bg="#ffffff").place(relx=0.5, y=220, anchor=tk.CENTER)
        
        tk.Button(item_frame, text="Edit", font=("Arial", 14, "bold"), fg="#000000", bg="#ffa214", activebackground="#e28700", bd=0, highlightthickness=0, 
                  command=lambda d=item: create_admin_edit_menu_page(root, bg_images, d, category)).place(x=25, y=250, width=70, height=35)
        ...
        tk.Button(item_frame, text="Delete", font=("Arial", 14, "bold"), fg="#000000", bg="#f11c0c", activebackground="#8e140b", bd=0, highlightthickness=0,
                  command=lambda mid=item[MENU_ID_IDX], name=item[MENU_NAME_IDX]: confirm_delete(mid, name)).place(x=105, y=250, width=70, height=35)
...
def create_admin_edit_page(root, bg_images, category='Drinks'):
    ...
    tk.Button(root, text="Add Menu", font=("Arial", 16, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=lambda: create_admin_add_menu_page(root, bg_images, category)).place(x=800, y=20, width=120, height=40)
    
    menu_item_frame = tk.Frame(root, bg="white", bd=0, highlightthickness=0, width=900, height=300); menu_item_frame.place(x=30, y=200)
    ...
    tk.Button(root, text="Drinks", **button_style, command=lambda: switch_category('Drinks')).place(x=300, y=150, width=120, height=40)
    tk.Button(root, text="Dessert", **button_style, command=lambda: switch_category('Dessert')).place(x=430, y=150, width=120, height=40)
    tk.Button(root, text="Food", **button_style, command=lambda: switch_category('Food')).place(x=560, y=150, width=120, height=40)

    display_admin_menu_items(root, category, bg_images, menu_item_frame)

    tk.Button(root, text="Back", font=("Arial", 16, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=lambda: create_admin_order_page(root, bg_images)).place(x=20, y=20, width=80, height=35)
...
def create_admin_order_page(root, bg_images):
    ...
    tk.Label(root, text="ADMIN", font=("Arial", 18, "bold"), fg="#003728", bg="white").place(x=850, y=20)
    
    table_names = ["1", "2", "3", "4"]; button_width = 150; button_height = 150; start_x = 100; spacing = 50
    ...
    for i, num in enumerate(table_names):
        ...
        tk.Button(root, text=f"Table\n{num}", font=("Arial", 24, "bold"), fg="#417c49", bg="#ebf3c7", activebackground="#a0b507", bd=0, highlightthickness=0, 
                  command=lambda n=num: messagebox.showinfo("Order View", f"เปิดดูออเดอร์โต๊ะ {n} (ยังไม่ถูกสร้าง)")).place(x=x, y=170, width=button_width, height=button_height)

    tk.Button(root, text="Log Out", font=("Arial", 16, "bold"), fg="#ffffff", bg="#417c49", activebackground="#2c5432", bd=0, highlightthickness=0, command=lambda: create_main_page(root, bg_images)).place(x=20, y=490, width=120, height=40)
    tk.Button(root, text="Sales", font=("Arial", 16, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=lambda: messagebox.showinfo("Sales", "เปิดรายงานยอดขาย (ยังไม่ถูกสร้าง)")).place(x=700, y=490, width=100, height=40)
    tk.Button(root, text="Menu", font=("Arial", 16, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=lambda: create_admin_edit_page(root, bg_images, 'Drinks')).place(x=820, y=490, width=100, height=40)
...
def display_menu_items(root, category, bg_images, item_container):
    for widget in item_container.winfo_children(): widget.destroy()
    menu_list = get_all_menu_items(category)
    col_width = 200; start_x = 70; start_y = 10 
    for i, item in enumerate(menu_list):
        col = i % 4; x = start_x + col * (col_width + 20)
        item_frame = tk.Frame(item_container, bg="#ebf3c7", width=col_width, height=250, bd=0, highlightthickness=0); item_frame.place(x=x, y=start_y)
        img_path = item[MENU_IMG_PATH_IDX]; photo = None
        if img_path and os.path.exists(img_path):
            img_resized = Image.open(img_path).resize((150, 150), Image.Resampling.LANCZOS); photo = ImageTk.PhotoImage(img_resized)
        if photo:
        ...
        tk.Label(item_frame, text=item[MENU_NAME_IDX], font=("Arial", 14, "bold"), fg="#003728", bg="#ebf3c7").place(relx=0.5, y=170, anchor=tk.CENTER)
        tk.Label(item_frame, text=f"{item[MENU_PRICE_IDX]} ฿", font=("Arial", 16, "bold"), fg="#417c49", bg="#ebf3c7").place(relx=0.5, y=210, anchor=tk.CENTER)
        
        tk.Button(item_frame, text="Add to Cart", font=("Arial", 12), command=lambda i=item: messagebox.showinfo("Cart", f"Added {i[MENU_NAME_IDX]} to cart")).place(relx=0.5, y=235, anchor=tk.CENTER, width=100, height=30)
...
def create_menu_page(root, bg_images, user_data, table_number):
    ...
    profile_button_image = load_and_display_profile_pic_on_button(pic_path, 'default_profile_pic_small', size=25) 
    ...
    profile_btn.image = profile_button_image; profile_btn.place(x=25, y=25, width=45, height=45) 
    
    tk.Label(root, text=f"โต๊ะ {table_number}", font=("Arial", 14, "bold"), fg="#003728", bg="#bbd106").place(x=75, y=35)
    tk.Label(root, text="🛒 Cart", font=("Arial", 14, "bold"), fg="#003728", bg="#bbd106").place(x=850, y=35)

    button_style = {"font": ("Arial", 16, "bold"), "fg": "#003728", "bg": "#bbd106", "activebackground": "#a0b507", "bd": 0, "highlightthickness": 0}
    def switch_category(category): display_menu_items(root, category, bg_images, menu_item_frame)
...
def create_table_page(root, bg_images, user_data): 
    ...
    def select_table(num):
        ...
        for i in range(4):
            ...
            if i + 1 == num: btn.config(bg="#a0b507")
    ...
    for i, num in enumerate(table_names):
        ...
        btn.place(x=x, y=170, width=button_width, height=button_height)
    ...
    tk.Button(root, text="Next", font=("Arial", 18, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=handle_next).place(x=380, y=390, width=200, height=45)
...
def create_profile_page(root, bg_images, user_data):
    ...
    *** ฟังก์ชันที่ถูกถามถึง (NameError) ***
    ...
    text_font = ("Arial", 18, "bold"); text_fg = "#003728"; text_bg = "#f3f6e8" 
    ...
    create_info_label(f"Frist Name : {first_name}", 200, 200, width=250)
    create_info_label(f"Last Name : {last_name}", 560, 200, width=250)
...
def create_edit_profile_page(root, bg_images, user_data):
    ...
    *** ฟังก์ชันที่ถูกเรียกใช้ใน create_profile_page ***
    ...
    tk.Entry(root, textvariable=first_name, font=entry_font, bd=1, relief=tk.FLAT).place(x=350, y=190, width=180, height=30)
    tk.Label(root, text="Last Name :", font=entry_font, fg="#003728", bg="white", anchor="w").place(x=540, y=190, width=140, height=30)
    tk.Entry(root, textvariable=last_name, font=entry_font, bd=1, relief=tk.FLAT).place(x=680, y=190, width=180, height=30)
    ...
    tk.Entry(root, textvariable=phone_number, font=entry_font, bd=1, relief=tk.FLAT, validate="key", validatecommand=vcmd).place(x=380, y=230, width=150, height=30)
    tk.Label(root, text="BirthDate :", font=entry_font, fg="#003728", bg="white", anchor="w").place(x=540, y=230, width=140, height=30)
    tk.Entry(root, textvariable=birth_date, font=entry_font, bd=1, relief=tk.FLAT).place(x=680, y=230, width=180, height=30)
    tk.Label(root, text="Email :", font=entry_font, fg="#003728", bg="white", anchor="w").place(x=200, y=270, width=150, height=30)
    tk.Entry(root, textvariable=email, font=entry_font, bd=1, relief=tk.FLAT).place(x=350, y=270, width=510, height=30)
...
# ... (ฟังก์ชันอื่น ๆ ที่เกี่ยวข้องทั้งหมดตามโค้ดเต็มก่อนหน้า) ...
...
def create_login_page(root, bg_images):
    ...
    tk.Button(root, text="Submit", font=("Arial", 18, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=handle_submit).place(x=380, y=340, width=200, height=45)
    tk.Button(root, text="Back", font=("Arial", 18, "bold"), fg="#003728", bg="#bbd106", activebackground="#a0b507", bd=0, highlightthickness=0, command=lambda: create_main_page(root, bg_images)).place(x=380, y=400, width=200, height=45)
...
def create_signup_page(root, bg_images):
    ...
    def handle_signup():
        ...
        if len(phone_number) != 10: messagebox.showerror("Sign Up Failed", "กรุณากรอกเบอร์โทรศัพท์ให้ครบ 10 หลัก")
        elif not (username and password and first_name and last_name): messagebox.showerror("Sign Up Failed", "กรุณากรอก Username, Password, ชื่อ และนามสกุล")
        else:
            ...
            else: messagebox.showerror("Sign Up Failed", "Username นี้ถูกใช้ไปแล้ว กรุณาเลือก Username ใหม่")
...
if __name__ == "__main__":
    ...
    # กำหนด Path รูปภาพ (กรุณาแก้ไข Path เหล่านี้ให้ถูกต้อง)
    image_paths = {
        ...
        'editdrinks': "D:\\โปรเจ็คของใบหม่อน\\editdrinks.png",
        'addmenu': "D:\\โปรเจ็คของใบหม่อน\\addmenu.png",
        'editmenu': "D:\\โปรเจ็คของใบหม่อน\\editmenu.png",
        'menudrink': "D:\\โปรเจ็คของใบหม่อน\\menudrink.png",
