...
def create_admin_edit_page(root, bg_images, category='Drinks'):
    ...
    # 4. สร้าง Frame *ภายใน* Canvas ที่จะใช้บรรจุเมนู (นี่คือ 'menu_item_frame' ใหม่)
    ...
    def on_frame_configure(event):
        canvas.configure(scrollregion=canvas.bbox("all")) # อัปเดตขนาดพื้นที่เลื่อน

    def on_canvas_configure(event):
        canvas.itemconfig(canvas_frame_id, width=event.width) # ปรับความกว้าง Frame ภายในให้เท่า Canvas

    menu_item_frame.bind("<Configure>", on_frame_configure)
    canvas.bind("<Configure>", on_canvas_configure)

    # 8. ผูก Event การเลื่อน Mouse Wheel (สำคัญมาก!)
    ...
    # 9. ส่ง Frame *ภายใน* (menu_item_frame) ไปให้ฟังก์ชัน display
    display_admin_menu_items(root, category, bg_images, menu_item_frame)

    # 10. ผูก Mouse Wheel กับ *ทุกอย่าง* ภายใน Frame หลังจากที่สร้างเมนูเสร็จแล้ว
...
def create_main_page(root, tk_images): # 1. เปลี่ยนชื่อตัวแปร
    for widget in root.winfo_children(): widget.destroy()
    
    # 2. บอก root ว่าจะใช้รูปไหนและ Label ไหน
    root.current_bg_key = 'main' 
    if 'main' not in tk_images: messagebox.showerror("Error", "ไม่พบรูปภาพ 'main'"); return
    
    background_label = tk.Label(root, image=tk_images['main'])
    background_label.place(x=0, y=0, relwidth=1, relheight=1)
    root.background_label = background_label # 3. บอก root ให้รู้จัก background_label

    # --- START RELATIVE PLACEMENT ---
    # คำนวณจาก 960x540
    
    # Log In Button: x=110, y=330, width=120, height=45
    # relx = 110 / 960 = 0.1145
    # rely = 330 / 540 = 0.6111
    # relwidth = 120 / 960 = 0.125
    # relheight = 45 / 540 = 0.0833
    tk.Button(root, text="Log In", bg="#bbd106", fg="#003728", font=("Arial", 16,"bold"), activebackground="#bbd106", bd=1, highlightthickness=0, command=lambda: create_login_page(root, tk_images)).place(
        relx=0.1145, rely=0.6111, relwidth=0.125, relheight=0.0833
    )

    # Sign Up Button: x=255, y=330, width=120, height=45
    # relx = 255 / 960 = 0.2656
    # rely = 330 / 540 = 0.6111
    # relwidth = 120 / 960 = 0.125
    # relheight = 45 / 540 = 0.0833
    tk.Button(root, text="Sign Up", bg="#bbd106", fg="#003728", font=("Arial", 16,"bold"), activebackground="#bbd106", bd=1, highlightthickness=0, command=lambda: create_signup_page(root, tk_images)).place(
        relx=0.2656, rely=0.6111, relwidth=0.125, relheight=0.0833
    )
    
    # --- END RELATIVE PLACEMENT ---

    # 4. ต้องส่ง tk_images ไปยังฟังก์ชันถัดไปด้วย
    create_nav_button(root, tk_images, lambda: create_main_page(root, tk_images))
...
if __name__ == "__main__":
    ...
    # 1. เก็บรูปต้นฉบับ (PIL) และรูปที่แปลงแล้ว (ImageTk) แยกกัน
    original_pil_images = {} # เก็บรูปต้นฉบับความละเอียดเต็ม
    tk_images = {}           # เก็บรูปที่ resize แล้วสำหรับ Tkinter
    
    # 2. เก็บตัวแปรสำคัญไว้ใน root
    root.current_bg_key = None
    root.background_label = None

    try:
        for name, path in image_paths.items():
            if os.path.exists(path):
                original_image = Image.open(path)
                original_pil_images[name] = original_image # เก็บต้นฉบับ
                
                # สร้างรูปเริ่มต้นขนาด 960x540
                if name not in ['default_profile_pic', 'default_profile_pic_small', 'default_menu_img']:
                    ...
                    tk_images[name] = ImageTk.PhotoImage(resized_image)
                else:
                    # รูป default ไม่ต้อง resize (หรือจัดการตามเดิม)
                    tk_images[name] = ImageTk.PhotoImage(original_image) 
            else:
                # ถ้าไม่เจอไฟล์สำคัญ (เช่น default) ให้แจ้งเตือน แต่ถ้าเป็นรูปพื้นหลัง อาจจะข้ามไปก่อนได้
                print(f"Warning: ไม่พบไฟล์รูปภาพ {path}")

    except Exception as e:
        messagebox.showerror("Error Loading Images", str(e)); root.destroy(); exit()

    
    # 3. ฟังก์ชันสำหรับ Resize ภาพพื้นหลัง
    def on_resize(event):
        # ทำงานเฉพาะเมื่อมี key และ label ที่กำหนดไว้เท่านั้น
        if not root.current_bg_key or not root.background_label:
            return

        key = root.current_bg_key
        
        # ป้องกัน error ถ้า key ไม่มีใน original_pil_images
        if key not in original_pil_images:
            return

        # 1. เอารูปต้นฉบับ
        original_pil = original_pil_images[key]
        
        # 2. หาขนาดหน้าต่างใหม่
        new_width = event.width
        new_height = event.height
        
        # 3. Resize รูปต้นฉบับตามขนาดหน้าต่างใหม่
        try:
            resized_pil = original_pil.resize((new_width, new_height), Image.Resampling.LANCZOS)
            new_tk_image = ImageTk.PhotoImage(resized_pil)
            
            # 4. อัปเดต Label
            root.background_label.config(image=new_tk_image)
            
            # 5. (สำคัญมาก) เก็บ reference กันรูปหาย
            root.background_label.image = new_tk_image
        except Exception as e:
            print(f"Error resizing image: {e}") # กันโปรแกรมแครชตอน resize เร็วๆ

    # 4. ฟังก์ชันสำหรับ Toggle Fullscreen (กด F11)
    ...
    # 5. สั่งให้โปรแกรมเริ่มแบบเต็มจอ (zoomed)
    root.state('zoomed') 
    
    # 6. ผูก (bind) event
    root.bind("<Configure>", on_resize) # <Configure> ทำงานทุกครั้งที่ขนาดเปลี่ยน
    root.bind("<F11>", toggle_fullscreen)
    root.bind("<Escape>", exit_fullscreen)

    # 7. เรียกหน้าแรก (ส่ง tk_images ไปแทน bg_images)
    create_main_page(root, tk_images)
